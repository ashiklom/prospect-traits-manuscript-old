...
Data for this analysis comes from several different projects, reported in \ref(projecttable).

```{r projecttable}

spectra_samples <- tbl(specdb, 'samples') %>%
...
#pander::pandoc.table(projecttable, 
                     #emphasize.strong.rows = nrow(projecttable),
                     #set.caption = 'Project information')
...
ggplot(sitemap_dat) + 
    aes(x = site_lon, y = site_lat, size = n, fill = projectcode) +
...
#biome_labs <- biomes %>%
    #group_by(biome) %>%
    #summarize(x = mean(prec), y = mean(temp)) 

biome_labs <- tribble(
    ~biome, ~x, ~y,
    'Boreal forest', 99.4, -0.417,
    'Subtropical desert', 42.78, 23.87,
    'Temperate forest', 160, 10,
    'Temperate grassland desert', 16.64, 6.86,
    'Temperate rain forest', 236, 12.5,
    'Tropical rain forest', 340, 24,
    'Tropical forest savanna', 150.53, 22.53,
    'Tundra', 20, -10,
    'Woodland shrubland', 85, 16)
plt <- ggplot() + 
    aes(x = prec, y = temp) +
    geom_polygon(data = biomes, aes(fill = biome), size = 2, alpha = 0.2) + 
    geom_point(data = sitemap_dat, aes(size = n, color = projectcode)) + 
    geom_label(data = biome_labs, 
                             aes(x = x, y = y, label = biome)) + 
