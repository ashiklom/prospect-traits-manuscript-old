...
```{r anovaplot, fig.cap = anovaplot_cap}
...
anova_dat <- Reduce((function(...) merge(..., by = 'variable')), anova_list) %>%
...
ggplot(anova_dat) + 
    ...
    geom_bar(stat = 'identity') + 
    scale_fill_manual(values = c(vars$color, 'grey'))
    
```

```{r traitsclimate, fig.cap = traitsclimate_cap}
...
```{r traitsclimatesp, fig.cap = traitsclimatesp_cap}

traitsclimatesp_cap <- paste('Relationship of traits with climate: Species means')
traitcli_plt %+% 
    (traitcli_dat %>% 
...
```{r traitsclimateres, fig.cap = traitsclimateres_cap}

traitsclimateres_cap <- paste('Relationship of traits with climate:', 
                              'Species mean residuals')
traitcli_plt %+% 
    (traitcli_dat %>% 
